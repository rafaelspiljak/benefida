...
export const Section = memo(
  ({ section, responses, handleChange, isActive }) => {
    return (
      <div
        className={`bg-[#D7EAD6] rounded shadow p-[30px] md:p-6 w-full ${
          isActive ? "opacity-100" : "opacity-30"
        }`}
      >
        <h2 className="md:hidden !text-[32px] !text-[#41553E] !text-left !font-medium !mb-6">
          {section.title}
        </h2>
        ...
        <table className="hidden md:table w-full border-collapse !border-spacing-y-2">
          <thead>
            <tr>
              <th className="!text-left !text-[44px] font-medium text-[#41553E] !pb-6 !leading-[100%] !tracking-[1.5%]">
                {section.title}
              </th>
              {options.map((opt) => (
                <th
